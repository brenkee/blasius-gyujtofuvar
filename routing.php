...
if (!class_exists('RoutingHttpException')) {
    class RoutingHttpException extends RuntimeException
    {
        private $statusCode;
        private $path;
        private $responseBody;

        public function __construct(string $message, int $statusCode, string $path, ?string $responseBody = null)
        {
...
if (!function_exists('routing_exception_context')) {
    function routing_exception_context(Throwable $e): array
    {
        $context = [];
        if ($e instanceof RoutingHttpException) {
            $context['status'] = $e->getStatusCode();
            $context['path'] = $e->getPath();
            $context['response'] = $e->getResponseBody();
        }
        return $context;
    }
}
...
if (!function_exists('routing_http_request')) {
    function routing_http_request(string $method, string $path, array $payload, array $cfg, int $timeout = 45): array
    {
        ...
        if ($status < 200 || $status >= 300) {
            routing_log_error('HTTP request failed', [
                'path' => $path,
                'status' => $status,
                'body' => $rawResponse !== '' ? $rawResponse : null,
            ]);
            throw new RoutingHttpException('routing_http_status_' . $status, $status, $path, $rawResponse);
        }
        $decoded = json_decode($rawResponse, true);
        if (!is_array($decoded)) {
            routing_log_error('Bad response body', [
                'path' => $path,
                'status' => $status,
                'body' => $rawResponse !== '' ? $rawResponse : null,
            ]);
    ...
    function routing_fetch_matrix(array $jobs, array $origin, array $cfg, bool $returnToOrigin): array
    {
        ...
        for ($sourceIndex = 0; $sourceIndex < $count; $sourceIndex++) {
            for ($destStart = 0; $destStart < $count; $destStart += $chunkSize) {
                ...
                $destChunk = array_values(array_filter($destChunk, static function ($idx) use ($sourceIndex) {
                ...
                if (!isset($response['durations']) || !is_array($response['durations'])) {
                    throw new RuntimeException('routing_matrix_missing_durations');
                }
...
if (!function_exists('routing_apply_route')) {
    function routing_apply_route(array $items, array $roundMeta, int $roundId, array $cfg): array
    {
        ...
        $fromCache = false;
        if (!$result) {
            $optimizationLimit = isset($cfg['routing']['optimization_max_jobs']) ? (int)$cfg['routing']['optimization_max_jobs'] : 50;
            $shouldSkipOptimization = $optimizationLimit > 0 && $jobCount > $optimizationLimit;
            if (!$shouldSkipOptimization) {
                try {
                    $result = routing_run_optimization($jobs, $origin, $cfg, $returnToOrigin);
                } catch (Throwable $e) {
                    $context = array_merge(
                        ['error' => $e->getMessage(), 'round' => $roundId],
                        routing_exception_context($e)
                    );
                    routing_log_error('Optimization failed', $context);
                    $result = null;
                }
            }
            $canUseMatrixFallback = $jobCount < 50;
            if (!$result && $canUseMatrixFallback) {
                try {
                    ...
                    routing_log_error('Matrix fallback failed', $context);
                    throw $e;
                }
            } elseif (!$result && !$canUseMatrixFallback) {
                routing_log_error('Matrix fallback skipped', ['round' => $roundId, 'job_count' => $jobCount]);
            }
            if ($result) {
